...
IMPLEMENTED_RELATIONS = [
    ...
    # Un-reserved by ticket 18 / plan G5b: an applied dominant (V/x, V7/x) points at the
    # diatonic triad it tonicises.  Implemented ONLY because the applied chords the engine
    # builds (ticket 17) each launch a real V7/x -> x resolution drill.
...
GENERATION_RULES = [
    ...
    # applied-dominant (secondary-dominant) template:
    "secondary_dominant_edges",     # each diatonic triad belongs_to_key; each V(7)/x -> its target
]
...
NODE_GENERATION_RULES = [
    ...
    "secondary_dominant_nodes",     # the applied dominants of the key's tonicisable degrees
]
        ...
        for ec in self.edge_classes:
            ...
            if ec.relation in RESERVED_RELATIONS and ec.implemented:
                raise ValueError(
                    ...
                    f"implemented")
            # Leaving RESERVED_RELATIONS must not lose the "no relation without content"
            # gate the reserved list provided (plan section 9.2).  ``secondary_dominant_of``
            # may only be declared implemented by a template that also *generates* it from
            # applied-dominant nodes -- and that node rule emits a node only where a
            # launchable drill exists.  Declaring the arrow without its producer is exactly
            # the hole the reserved list used to close.
            if ec.relation == "secondary_dominant_of" and ec.implemented:
                if "applied_dominant" not in seen_kinds:
                    raise ValueError(
                        "a template implementing 'secondary_dominant_of' must declare the "
                        "'applied_dominant' node class: the relation's source is an applied "
                        "chord, never a diatonic node")
                for rule, field in (("secondary_dominant_nodes", self.node_generation_rules),
                                    ("secondary_dominant_edges", self.generation_rules)):
                    if rule not in field:
                        raise ValueError(
                            f"a template implementing 'secondary_dominant_of' must run the "
                            f"{rule!r} rule, which is what gates the relation on a launchable "
                            f"drill; declaring the relation without it would assert a "
                            f"tonicisation the app cannot practise")
    ...
    edge_classes = [
        ...
        EdgeClass("atlas_node_available", "Atlas node available", directed=True,
                  ...
                  description="Points at the related Interactive Harmony Atlas "
                              "concept node."),
        # Reserved vocabulary -- named now, generated by a future layer.  (The
        # applied-dominant relation left this list in ticket 18: it is implemented
        # by ``secondary_dominant_network_v1``, which owns the V(7)/x -> x edge.)
...
def secondary_dominant_network_v1() -> HarmonicNetworkTemplate:
    """The key's tonicisable degrees and the applied dominants that tonicise them.

    The first *chromatic* topology (ticket 18 / plan G5b).  A key-centre anchor holds the seven
    diatonic triads on the outer ring; the applied dominants (``V/x`` outside, ``V7/x`` inside
    each target's spoke) sit on their own ring and point at the triad they tonicise with the
    now-implemented ``secondary_dominant_of`` relation.

    Two honesty properties make the flip legitimate (plan section 9.2):

    * an applied dominant is **not** in the key -- it carries no ``belongs_to_key`` edge, so the
      graph itself shows why ``D7`` is the intruder in C major;
    * the relation is generated **only** where a launchable drill exists: every applied node
      carries the real ``V(7)/x -> x`` resolution drill the engine builds (ticket 17), and a node
      whose drill will not compile is not emitted at all -- so no edge can outrun the content.

    Two of the seven degrees never receive an applied node: the tonic (its applied dominant is
    simply the home ``V``) and the diminished degree (a diminished triad cannot act as a momentary
    tonic).  That absence is the lesson, drawn rather than asserted.
    """
    node_classes = [
        NodeClass("key_center", "Key centre", "slate", "#64748b",
                  "The home key. Applied dominants are heard against it -- they do not live in it.",
                  semantic_level="key", entity_role="anchor", canonical_ref="scale"),
        NodeClass("diatonic_triad", "Diatonic triad", "teal", "#14b8a6",
                  "One of the seven diatonic triads; five of them can be tonicised.",
                  semantic_level="chord", entity_role="instance", canonical_ref="triad"),
        NodeClass("applied_dominant", "Applied dominant", "rose", "#f43f5e",
                  "A chromatic V/x or V7/x: the dominant OF a diatonic degree, borrowed from the "
                  "key that degree would be the tonic of.",
                  semantic_level="chord", entity_role="instance", canonical_ref="triad"),
    ]
    ...
    edge_classes = [
        EdgeClass("belongs_to_key", "Belongs to key", directed=True,
                  visual_class="membership", default_visible=True,
                  description="A diatonic triad belongs to the home key. Applied dominants "
                              "deliberately have no such edge -- that is what makes them chromatic."),
        EdgeClass("secondary_dominant_of", "Secondary dominant of", directed=True,
                  visual_class="applied", default_visible=True,
                  description="An applied dominant tonicises its target: V7/V → V treats the "
                              "dominant as a momentary tonic."),
    ]

    layout_rules = [
        LayoutRule("key_center", "central", radius=0.0, angle_offset=0.0, node_radius=28.0),
        LayoutRule("diatonic_triad", "outer", radius=300.0, angle_offset=0.0, node_radius=19.0),
        LayoutRule("applied_dominant", "inner", radius=185.0, angle_offset=0.0, node_radius=18.0),
    ]
    ...
    return HarmonicNetworkTemplate(
        ...
        description=(
            "Applied dominants (V/V, V7/ii, ...) and the diatonic degrees they tonicise. Each "
            "applied chord points at its target with the secondary_dominant_of relation and "
            "launches the real resolution drill; only the diatonic triads are wired to the key "
            "centre, so the chromatic chords read as the outsiders they are."),
        node_classes=node_classes,
        edge_classes=edge_classes,
        layout_rules=layout_rules,
        generation_rules=[
            # The key-membership half is the core template's rule verbatim (this template
            # declares no function families, so its member_of_function half is vacuous).
            "diatonic_membership_edges",
        ...
        context_schema={"key": "C", "mode": "major"},
        center_keys=["C"],
        node_layers=["key_center", "diatonic_triad", "applied_dominant"],
        launch_rules={
            "launchable_kinds": ["key_center", "diatonic_triad", "applied_dominant"],
            "reserved_kinds": [],
            "reserved_reason": "",
        },
    )
