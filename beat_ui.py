...
function beatEdges(absIdx){
  ...
  const times = noteAnchorsWithTime(g);       // [{x,t}, ...]
  const edges = [rr.left];

  if (times.length) {
    // Boundary between beat k and k+1 sits midway between the *engraved* k.* and (k+1).* note columns
    for (let k = 1; k < beats; k++) {
      const leftXs  = times.filter(o => o.t >= k && o.t < k + 1).map(o => o.x);
      const rightXs = times.filter(o => o.t >= k + 1 && o.t < k + 2).map(o => o.x);

      let xl = leftXs.length  ? median(leftXs)  : rr.left + (k / beats) * (rr.right - rr.left);
      let xr = rightXs.length ? median(rightXs) : rr.left + (k / beats) * (rr.right - rr.left);

      let e = (xl + xr) / 2;
      e = Math.max(edges[edges.length - 1] + 1, Math.min(rr.right - 1, e)); // strictly increasing
      edges.push(e);
    }
  } else {
    // Fallback: quantiles over plain note x-positions
    const xs = noteAnchorsInMeasure(g);
    const width = rr.right - rr.left;
    for (let k = 1; k < beats; k++) {
      const ideal = rr.left + (k / beats) * width;
      const mids = [];
      for (let j = 0; j < xs.length - 1; j++) mids.push((xs[j] + xs[j + 1]) / 2);
