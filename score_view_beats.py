...
.beatBox.sel {
    outline: 3px solid #ff3b30;                 /* stronger border when selected */
    background: rgba(255, 214, 10, 0.45);       /* brighter fill when selected */
  }
...
<script>
  ...
  const ABS_INDEXES    = {ABS_INDEXES_JSON};
    ...
    const BEAT_TIMES_MAP = {BEAT_TIMES_MAP_JSON};   // ← add this
  ...
  // --- Note / sidebar helpers (no pitch labels) -----------------------

  // highlight state is driven only from the sidebar
  ...
  function highlightOneNote(node, on=true) {
    if (!node) return;
    if (on) {
      node.classList.add("note-hl");
      applyDirectHighlight(node, true);
    } else {
      node.classList.remove("note-hl");
      applyDirectHighlight(node, false);
    }
  }

  function clearAllNoteHighlights() {
    const svg = svgRoot(); if (!svg) return;
    svg.querySelectorAll(".note-hl").forEach(n => highlightOneNote(n, false));
  }
  ...
  // collect note group <g> elements whose visual x-center lies inside [L,R]
...
function rebuildSidebar() {
  ...
  boxes.forEach((box) => {
    ...
    const g   = measureGroupByAbs(abs); if (!g) return;

    const br = box.getBoundingClientRect();
    const notes = noteGroupsWithin(g, br.left, br.right, true);  // sorted L→R
    const beat  = beatIndexForBox(abs, br);

    const labels = (PITCH_MAP[String(abs)] && PITCH_MAP[String(abs)][String(beat)]) || [];
    ...
    // Pair notes with labels left→right; allow different counts gracefully
    const n = Math.max(notes.length, labels.length);
    for (let i = 0; i < n; i++) {
      const node = notes[i];
      const lab  = labels[i] || "";
...
function _ensureBeatBoxes(){
  ...
  for (const abs of ORDER_ABS){
    ...
    div.dataset.abs = String(abs);           // ← so we know which measure this is

    // toggle selection for THIS box
    div.addEventListener('click', (ev) => {
      ev.stopPropagation();
      div.classList.toggle('sel');
      rebuildSidebar();                      // update sidebar + highlights
    });
...
function beatIndexForBox(abs, box) {
  const xs = ANCHORS_BY_ABS[abs] || [];
  if (!xs.length) return 1;

  const first = xs[0], last = xs[xs.length - 1];
  const span  = Math.max(1, last - first);

  const beats = (BEAT_TIMES_MAP[abs] ? BEAT_TIMES_MAP[abs].length : 1); // ← use grid we injected
  const cx = (box.left + box.right) / 2;
  const u  = Math.min(0.9999, Math.max(0, (cx - first) / span));
  const k  = Math.floor(u * beats);   // 0..beats-1
  return Math.max(1, Math.min(beats, k + 1));
}
...
class ScoreViewBeats(QWidget):
    ...
    def _load_page(self, page: int):
        ...
        # build injection arrays (page-specific)
        ...
        # Robustly handle different extract_pitches return shapes
        ret = extract_pitches(self.xml_path)
        beats_map = {}
        if isinstance(ret, dict):
            # already the beats map
            beats_map = ret
        elif isinstance(ret, (list, tuple)):
            # common cases: (rows, beats_map), (rows, beats_map, meta), (beats_map,)
            for item in ret:
        ...
        beats_map_json = json.dumps({str(k): {str(b): v for b, v in d.items()} for k, d in beats_map.items()})
        html = (
            _HTML
            .replace("{ABS_INDEXES_JSON}", json.dumps(abs_indexes))
            .replace("{NOTE_TIMES_MAP_JSON}", json.dumps(note_times_map))
            .replace("{BEAT_TIMES_MAP_JSON}", json.dumps(beat_times_map))  # ← add this
            .replace("{PITCH_MAP_JSON}", beats_map_json)
        )
