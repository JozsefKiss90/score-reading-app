...
| `secondary_dominant_path` | An applied token in the pattern, or Lab `applied_chord` | The diatonic row with the chromatic intruder **lifted out of it**: only the diatonic markers link to the key anchor, and the applied chord's `secondary_dominant_of` arrow points at the degree it tonicises (drawn only where `secondary_dominant_network_v1` supports the pair — an applied *leading-tone* chord, `vii°7/x`, is drawn on the same evidence one step removed: that network's own arrows say the target is tonicisable, and the engine built this chord as the tonicisation of exactly that degree). The node's role label **and the edge's relation** follow the head — `vii°7/V` points at its target under `applied_leading_tone_of`, never `secondary_dominant_of`, because the relation id is learner-visible in the detail panel. No other scene may claim an applied chord. |
